...
contract('NaanBar', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.naan = await NaanToken.new({ from: alice });
        this.bar = await NaanBar.new(this.naan.address, { from: alice });
        this.naan.mint(alice, '100', { from: alice });
        this.naan.mint(bob, '100', { from: alice });
        this.naan.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.naan.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.naan.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.naan.approve(this.bar.address, '100', { from: alice });
        await this.naan.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.naan.balanceOf(this.bar.address)).valueOf(), '30');
        // NaanBar get 20 more NAANs from an external source.
        await this.naan.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more NAANs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.naan.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.naan.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.naan.balanceOf(bob)).valueOf(), '98');
    });
});
