...
contract NaanUni is Ownable {
    ...
    // Block number of distributing NAAN period ends.
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256 _naan, uint256 _uni) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        _naan = user.amount.mul(accNaanPerShare).div(1e12).sub(user.naanRewardDebt);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pendingNaan = user.amount.mul(pool.accNaanPerShare).div(1e12).sub(user.naanRewardDebt);
        uint256 pendingUni = user.amount.mul(pool.accUniPerShare).div(1e12).sub(user.uniRewardDebt);
        user.amount = user.amount.add(_amount);
        user.naanRewardDebt = user.amount.mul(pool.accNaanPerShare).div(1e12);
